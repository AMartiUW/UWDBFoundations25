...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_AMartinez')
	 Begin 
	  Alter Database [Assignment07DB_AMartinez] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_AMartinez;
	 End
	Create Database Assignment07DB_AMartinez;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_AMartinez;
...
SELECT
	...
	PreviousMonthCount = ISNULL(LAG(SUM(InventoryCount),1,0) 
			OVER(PARTITION BY ProductName 
		ORDER BY MONTH(InventoryDate)),0)
FROM  vProductInventories
